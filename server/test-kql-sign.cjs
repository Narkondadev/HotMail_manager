...
async function test() {
    ...
    // Look at last 24 hours for "sign in "
    const pastDate = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
    const kqlQuery1 = `subject:'sign in ' -from:microsoft.com -from:accountprotection.microsoft.com received>=${pastDate}`;
    ...
    console.log("Emails found for 'sign in ':", data1.value ? data1.value.length : 0);
